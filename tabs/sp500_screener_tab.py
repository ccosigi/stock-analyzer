    ...
    if st.button("🔍 S&P500 스크리닝 실행", type="primary", use_container_width=True):
        ...
        if qualifying_stocks:
            ...
            for i, stock in enumerate(qualifying_stocks[:3], 1):
                stock_info = get_stock_info(stock['ticker'])
                st.markdown(f"""
                <div class="stock-highlight" style="color: #000;">
                    <h4 style="color: #000;">#{i} {stock['ticker']} - {stock_info['name']}</h4>
                    <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-top: 10px; color: #000;">
            ...
            if avg_rsi < 30 and avg_bb_pos < 0.3:
                ...
                st_text = f"<p style='color: #000;'>평균 RSI({avg_rsi:.1f})와 BB Position({avg_bb_pos:.3f})이 모두 매우 낮습니다.</p><p style='color: #000;'>💡 <strong>전략</strong>: 단계적 매수를 통해 리스크를 분산하고, 반등 시점을 노려보세요.</p>"
            elif avg_rsi < 35 and avg_bb_pos < 0.4:
                sc, st_title = "result-box", "🟡 매수 고려 신호"
                st_text = f"<p style='color: #000;'>과매도 상태에 근접해 있습니다 (평균 RSI: {avg_rsi:.1f}, BB Position: {avg_bb_pos:.3f}).</p><p style='color: #000;'>💡 <strong>전략</strong>: 추가적인 기본 분석과 함께 신중한 매수를 고려해보세요.</p>"
            else:
                sc, st_title = "metric-container", "⚖️ 신중한 접근"
                st_text = f"<p style='color: #000;'>조건을 만족하지만 극도의 과매도는 아닙니다 (평균 RSI: {avg_rsi:.1f}).</p><p style='color: #000;'>💡 <strong>전략</strong>: 시장 상황과 개별 종목 펀더멘털을 종합적으로 검토하세요.</p>"

            st.markdown(f'<div class="{sc}" style="color: #000;"><h4 style="color: #000;">{st_title}</h4>{st_text}</div>', unsafe_allow_html=True)
